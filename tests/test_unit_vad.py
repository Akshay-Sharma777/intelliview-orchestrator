...
def test_normal_speech_detection():
    ...
    assert all(e.event_type in ("SPEECH_START", "SPEECH_CONTINUED") for e in events)
...
def test_sustained_silence_emits_single_end_of_speech():
    ...
    sustained_silence = _generate_synthetic_frames(duration_ms=1500, amplitude=0.0)
...
def test_deterministic_timestamps_from_sample_counts():
    ...
    detector = VoiceActivityDetector()
    audio = _generate_synthetic_frames(300, amplitude=0.1) + _generate_synthetic_frames(
        1500, amplitude=0.0
    )
